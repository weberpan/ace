...
Editor.prototype.indexFromPos = function(pos) {
    var doc = this.session.doc;
    var lines = doc.$lines || doc.getAllLines();
    var newlineLength = doc.getNewLineCharacter().length;
    var index = 0;
...
AceEmmetEditor.prototype = {
	...
	getSelectionRange: function() {
		...
		return {
			start: this.ace.indexFromPos(range.start),
			end: this.ace.indexFromPos(range.end)
		};
	},
	...
	createSelection: function(start, end) {
        this.ace.selection.setRange({
			start: this.ace.posFromIndex(range.start),
			end: this.ace.posFromIndex(range.end)
		});
    },
	...
	getCurrentLineRange: function() {
        ...
        var index = this.ace.indexFromPos({row: row, column: 0});
		return {
			start: index, 
	...
	getCaretPos: function(){
        ...
        return this.ace.indexFromPos(pos);
    },
	...
	replaceContent: function(value, start, end, noIndent) {        
        ...
        var range = this.ace.getSelectionRange();
        range.start = this.ace.posFromIndex(start);
        range.end = this.ace.posFromIndex(end);
        
        this.ace.session.replace(range, value);
        
        range.start = this.ace.posFromIndex(firstTabStop.start);
...
// runEmmetCommand("expand_abbreviation_with_tab", editor)
